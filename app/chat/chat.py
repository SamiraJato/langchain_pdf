...
def select_component(component_type, component_map, chat_args):
    # busca componentes registrados anteriormente
    components = get_conversation_components(chat_args.conversation_id)

    previous_component = components[component_type]


    if previous_component: # se ja tinha registrado, utiliza o mesmo
        builder = component_map[previous_component]
        return previous_component, builder(chat_args)
    else: # conversa nova, escolhe um componente daquele tipo de forma aleatoria considerando os scores
